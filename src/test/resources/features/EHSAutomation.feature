...
Background:
  ...
  Scenario: Verify to be able to see all available products as an Admin
    ...
    And click all items open button respectively then verify to see all infos and get product ids to store
  ...
  Scenario:Verify to be able to search product and see details directly as an Admin
    When Search EHSItem 1 with productId and click find button
